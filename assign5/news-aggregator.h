 ...
 */
  ...
  typedef std::pair<Article, Tokens> ArticleInfo;
  ...
  std::set<url> articleUrls;
  mutable std::mutex articleUrlsMutex;
  std::map<server, std::map<title, ArticleInfo>> articles;
  mutable std::mutex articlesMutex;
/**
 ...
 */
  ...
  bool containsArticleUrl(const Article& article) const;
  void downloadArticle(const Article& article, Tokens& tokens);
  void updateArticle(const Article& article, const Tokens& tokens);
  void processArticle(const Article& article);
  void processFeed(const url& feedUrl, const title& feedTitle);
/**
